...
const wordSEARCHBAR = 'coco';
const choices = data;
console.log('Recipes', choices.recipes[0].name);
console.log('Recipes.length', choices.recipes.length);
let result = [];

if (wordSEARCHBAR && wordSEARCHBAR.length > 2) {
  for (let i = 0; i < choices.recipes.length; i++) {
    if (
      choices.recipes[i].name.toLowerCase().includes(wordSEARCHBAR) ||
      choices.recipes[i].description.toLowerCase().includes(wordSEARCHBAR)
    ) {
      result.push(choices.recipes[i]);
    } else {
      for (let j = 0; j < choices.recipes[i].ingredients.length; j++) {
        if (
          choices.recipes[i].ingredients[j].ingredient
            .toLowerCase()
            .includes(wordSEARCHBAR)
        ) {
          result.push(choices.recipes[i]);
        }
      }
